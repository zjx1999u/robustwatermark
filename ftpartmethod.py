"""ft my robust watermark method model with use percent of trainset"""
...
parser.add_argument('--wm_batch_size', default=32, type=int, help='the batch size')
parser.add_argument('--max_epochs', default=20, type=int, help='the maximum number of epochs')
parser.add_argument('--load_path', default='./checkpoint/model.pth', help='the path to the pre-trained model')
...
if args.dataset == 'cifar10' or args.dataset == 'cifar100' :
    ...
    allnum=100000
lenofbatch = int((allnum+B+1)/B)

a=[i for i in range(lenofbatch)]
print(lenofbatch)
print("\na:")
print(a)
number = int(lenofbatch*usepercent)
sample_index =random.sample(a,number)
print(sample_index)
print(len(sample_index))
    
batch_train = (np.random.random(lenofbatch) <= -1)

for i in range(number):
    vali_index = sample_index[i]
    if(vali_index>=lenofbatch):
        print("not valid ")
        print(vali_index)
