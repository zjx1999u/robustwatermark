...
from trainer import test, train,train_L2,train_Logits_snnl,testSR
...
parser.add_argument('--snnl_weight', default=0.1, type=float, help='the batch size')   
...
# start training
temper = 10
snnl_weight = args.snnl_weight
for epoch in range(start_epoch, start_epoch + args.max_epochs):
   
    with open(logfile, 'a') as f:
        f.write('epoch : %d  lr: %f\n' % (epoch,lr_now))

    train_Logits_snnl(epoch, net, teachernet, criterion, optimizer, logfile, trainloader, device, lmbda_d,temper,snnl_weight, wmloader, tune_all=True)
    #train_Logits_snnl(epoch, net, teachernet, criterion, optimizer, logfile, loader, device,  lmbda_d , wmloader=False, tune_all=True,temprature=10,snnl_weight=0.1)
    print("Test acc:")
